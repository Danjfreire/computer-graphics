...
public class ScanLine {
    ...
    public List<Vector3> rasterize(List<Triangle> triangles, List<Vector3> projectedVectors, int screenW, int screenH) {
        ...
        for (Triangle t : triangles) {
            ...
            if(v2.getY() == v3.getY()){
                ...
                bottomFlatTriangle(v1,v2,v4,result);
                topFlatTriangle(v2,v4,v3,result);
            }
    ...
    private void topFlatTriangle(Vector3 v1, Vector3 v2, Vector3 v3, List<Vector3> result) {
        double slopeMin = (v3.getY() - v1.getY()) / (v3.getX() - v1.getX());
        double slopeMax = (v3.getY() - v2.getY()) / (v3.getX() - v2.getX());

        double xMin = v3.getX();
        double xMax = v2.getX();

        for( double scan = v3.getY() ; scan > v1.getY(); scan--){
            double aux = xMin;
            while(aux <= xMax ){
...
//        }else{
//            slope1 = (v3.getX() - v1.getX()) / (v3.getY() - v1.getY());
//            slope2 = (v2.getX() - v1.getX()) / (v2.getY() - v1 .getY());
//        }
...
//        System.out.println(v1.getY()+ ", " + v2.getY());
        for( double scan = v1.getY() ; scan <= v2.getY(); scan++){
            double aux = xMin;
            while(aux <= xMax){
                result.add(new Vector3(aux, scan,0));
                aux++;
